...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                             ...
                             QCompleter, QTabWidget, QFileDialog)
...
class DataLoaderThread(QThread):
    ...
    def run(self):
        ...
        if total_files == 0:
            ...
            self.finished.emit(mulan, [], audio_embeddings)
            return
        ...
        def preprocess_audio(filename):
            ...
            if os.path.exists(cache_path):
                ...
                return filename, embeds, cache_path, "cached"

            # Create a temporary file for the normalized output
            import tempfile
            import subprocess
            temp_fd, out_path = tempfile.mkstemp(suffix='.mp3')
            os.close(temp_fd) # Close file descriptor, ffmpeg will rewrite the file
            ...
            try:
                ...
                try:
                    ...
                    pass # ffmpeg not installed, soundfile will have to deal with the original file
                
                audio_to_load = out_path if os.path.exists(out_path) and os.path.getsize(out_path) > 0 else in_path
                
                wav, sr = sf.read(audio_to_load)
                
                if len(wav.shape) > 1:
                    wav = wav.mean(axis=1) # Convert to mono
                    
                # Standardize normalization max values 
                max_val = np.max(np.abs(wav))
                if max_val > 0:
                    wav = wav / max_val
                    
                return filename, torch.tensor(wav, dtype=torch.float32), cache_path, "extracted"
            except Exception as e:
                print(f"Error handling {filename}: {e}")
                return filename, None, cache_path, "error"
            finally:
        ...
        if total_files > 0:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                futures = {executor.submit(preprocess_audio, f): f for f in raw_files}

                completed = 0
                for future in tqdm(as_completed(futures), total=total_files, desc="Processing files"):
                    song_name, data, cache_path, source = future.result()
                    
                    if source == "cached" and data is not None:
                        audio_embeddings[song_name] = data
                        completed += 1
                        self.progress.emit(f"Loaded cached: {song_name}")
                        self.progress_percent.emit(int((completed / total_files) * 100))
                    elif source == "extracted" and data is not None:
                        batch_tensors.append(data)
                        batch_metadata.append((song_name, cache_path))
                        ...
                        if len(batch_tensors) >= BATCH_SIZE:
                            ...
                            for idx, (s_name, c_path) in enumerate(batch_metadata):
                                em = embeds[idx].unsqueeze(0).cpu() # match existing format
                                torch.save(em, c_path)
                                audio_embeddings[s_name] = em
                                completed += 1
                                self.progress.emit(f"Processed: {s_name}")
                                self.progress_percent.emit(int((completed / total_files) * 100))
                                
                            batch_tensors.clear()
                            batch_metadata.clear()
                            
                # Process any remaining files in the final smaller batch
        ...
        self.progress_percent.emit(100)
        self.finished.emit(mulan, raw_files, audio_embeddings)
...
class MusicApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.search_tabs.addTab(upload_tab, "Upload Audio")
    ...
    def combine_song_embeddings(self, song_names):
        ...
        return self.combine_embeddings(embeds)

    def combine_embeddings(self, embeds):
        if not embeds:
            return None
    ...
    def upload_and_search(self):
        ...
        file_paths, _ = QFileDialog.getOpenFileNames(self, "Select Audio Files", "", "Audio Files (*.wav *.mp3 *.flac *.ogg *.m4a)")
        if not file_paths:
            return
            
        embeddings = []
        for file_path in file_paths:
            self.status_label.setText(f"Processing uploaded file: {os.path.basename(file_path)}...")
            QApplication.processEvents()
            ...
            try:
                ...
                cmd = ['ffmpeg', '-y', '-i', file_path, '-ar', '24000', '-b:a', '128k', out_path]
                try:
                    subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                except FileNotFoundError:
                    pass
                    
                audio_to_load = out_path if os.path.exists(out_path) and os.path.getsize(out_path) > 0 else file_path
                ...
                self.status_label.setText(f"Extracting embedding for {os.path.basename(file_path)}...")
                QApplication.processEvents()
                
                with torch.no_grad():
                    embed = self.mulan(wavs=wav_tensor)
                    
                embed = embed[0].unsqueeze(0).cpu() # [1, vector_size] mapping
                embeddings.append(embed)
                
            except Exception as e:
                import traceback
                traceback.print_exc()
                self.status_label.setText(f"Error processing uploaded file: {e}")
            finally:
                if os.path.exists(out_path):
                    try:
                        os.remove(out_path)
        ...
        if not embeddings:
            self.status_label.setText("No valid embeddings could be extracted.")
            return
            
        self.status_label.setText("Combining embeddings...")
        QApplication.processEvents()
        
        target_embed = self.combine_embeddings(embeddings)
        
        self.status_label.setText("Ready!")
        self.rank_and_display(target_embed, None)
        ...
        try:
            ...
            for song_name, embed in self.audio_embeddings.items():
                ...
                def calc_sim(target_embed, e):
                    target_embed = target_embed.to(e.device)
